...
namespace dotnetapp.Services
{
    public class AuthService : IAuthService
    {
        ...
        /// <summary>
        /// The only role names this API recognises. Registration used to pass the
        /// client's string straight to RoleManager, which created any role it was
        /// handed — so the casing was unconstrained ("admin" and "Admin" became two
        /// distinct roles, and [Authorize(Roles="Admin")] matches case-sensitively).
        /// Anything unrecognised collapses to User.
        /// </summary>
        private static string CanonicalRole(string? requested) =>
            string.Equals(requested?.Trim(), "Admin", StringComparison.OrdinalIgnoreCase)
                ? "Admin"
                : "User";
